...
export const getAll = async(req,res,next)=>{
   ...
   // const products = await productModel.find({status:"Active"}).select('name price mainImage')//.skip(skip).limit(limit)
    const mongooseQuery =  productModel.find({status:"Active"}).select('name price mainImage')//.skip(skip).limit(limit)
    if(req.query.search){
    mongooseQuery.find({
        $or:[
...
export const getActive= async (req,res,next)=>{
   ...
   /* const checkcategory=await categoryModel.findById(queryObj.categoryId)
    ...
    .populate({
        ...
        match: { status: 'Active' },
        select: 'status'
    }).select('name price mainImage status')//.skip(skip).limit(limit)
    if(req.query.search){
    mongooseQuery.find({
        $or:[
       { name:{$regex:req.query.search}},
